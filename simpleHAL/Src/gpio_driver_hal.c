...
void gpio_enable_clock_peripheral (GPIO_Handler_t *pGPIOHandler){
	...
	//Verificamos para GPIOA
		...
		else if (pGPIOHandler-> PIOx == GPIOE) {
 ...
 * -OpenDrain
 * */
void gpio_config_otuput_type (GPIO_Handler_t *pGPIOHandler){
...
void gpio_config_output_speed (GPIO_Handler_t *pGPIOHandler){
	...
	//Limpiamos la posicio antes de cargar la nueva configuracion
...
void gpio_config_pullup_pulldown (GPIO_Handler_t *pGPIOHandler){
	...
	auxConfig = (pGPIOHandler->pinConfig.GPIO_pinPudControl << 2*pGPIOHandler->pinConfig.GPIO_PinNumber);
...
void gpio_WritePin (GPIO_Handler_t * pPinHandler, uint8_t newState){
	...
	if (newState == SET){
		...
		pPinHandler->pGPIOx->BSRR |= (SET << (pPinHandler->pinnConfig.GPIO_PinNumber + 16));
	}
}
